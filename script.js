...
// 1. Model data diubah menjadi array nominal pembayaran (default isi 0)
...
// 12. getNextUnpaidIndex() mencari cicilan yang nominalnya belum mencapai target
...
// 13. isOverdue() menggunakan status nominal pembayaran (pembayaran[index] < cicilan[index])
...
function render() {
  ...
  // 4. Perhitungan total pembayaran berdasarkan penjumlahan nominal yang dibayar
  let totalBayar = 0;
  ...
  tanggal.forEach((tgl, index) => {
    ...
    // 2. Mengganti penggunaan .includes() menjadi pengecekan nominal (>= cicilan)
    ...
    // Tombol batalkan memanggil toggle() langsung, tombol bayar memanggil konfirmasi modal modal terlebih dahulu
    const payAction = sudah ? `toggle(${index})` : `handlePayClick(${index})`;

    // 14. Menampilkan Target Cicilan & Nominal yang sudah dibayar pada setiap kartu
    card.innerHTML = `
      <div class="info">
  ...
  document.getElementById("sudahBayar").innerHTML = rupiah(totalBayar);
  
  // 5. Sisa hutang dihitung berdasarkan total nominal yang sudah dibayar
  document.getElementById("sisaHutang").innerHTML = rupiah(TOTAL_HARGA - totalBayar);

  // 3. Perbaikan perhitungan progress menggunakan filter nominal yang valid lunas, bukan .length
  const cicilanLunas = pembayaran.filter((nominal, idx) => nominal >= cicilan[idx]).length;
  
  document.getElementById("progressText").innerHTML = cicilanLunas + " / 16 Cicilan";

  const progressPct = (cicilanLunas / 16) * 100;
  document.getElementById("progressBar").style.width = progressPct + "%";
  document.getElementById("progressPercentage").textContent = Math.round(progressPct) + "%";

  updateDateBar();
}

// 6. Memperbaiki fungsi toggle() agar bersih dari nested function internal
function toggle(index) {
  const sudah = pembayaran[index] >= cicilan[index];

  if (sudah) {
    // Aksi pembatalan pembayaran
    ...
    // 7. Mengubah tombol Bayar agar menerima input nominal pembayaran lewat prompt
    const inputNominal = prompt(
      "Masukkan nominal pembayaran untuk Cicilan " + (index + 1) + ":\n(Minimal target: " + rupiah(cicilan[index]) + ")",
      cicilan[index]
    );

    // Jika user menekan batal pada prompt
    if (inputNominal === null) return;

    const nominal = Number(inputNominal);

    // 8. Validasi nominal tidak boleh kosong/bukan angka, dan minimal sebesar target cicilan
    ...
    // 9. Nominal yang lebih besar dari target diizinkan otomatis lolos ke sini
    pembayaran[index] = nominal;
  }

  // 10. Menyimpan nominal pembayaran terbaru ke localStorage
...
function executePayment() {
  ...
  // Ambil data status terbaru setelah toggle berjalan untuk feedback sukses
  if (pembayaran[index] >= cicilan[index]) {
    showSuccessFeedback(index);
  }
  
  // 11. Perbaikan executePayment() untuk mengecek semua lunas menggunakan .every()
  const semuaLunas = pembayaran.every((nominal, idx) => nominal >= cicilan[idx]);
  if (semuaLunas) {
    launchConfetti();
  }
}
...
document.getElementById("confirmOk").addEventListener("click", executePayment);
document.getElementById("confirmModal").querySelector(".modal-backdrop").addEventListener("click", closeConfirmModal);
